...
procedure LoadBahanMentah (var bahanMentah : tabmentah); {Membaca file eksternal yang bernama 'bahanmentah.txt' terus dimasukkin ke tipe bentukan mentah}
	...
	var {KAMUS LOKAL LoadBahanMentah}
		...
		temp			: string;  {Menyimpan data pada file eksternal secara sementara}
		inf				: textfile;
	...
	begin {ALGORITMA UTAMA LoadBahanMentah}
		...
		begin		
			...
			guardMark := pos ('|', temp);
			bahanMentah.tab[j].nama:= copy (temp,1,guardMark-2);
			{Membaca harga bahan mentah yang bertipe integer}
			temp := copy (temp,guardMark+2, length(temp));
			guardMark := pos ('|', temp);
			bahanMentah.tab[j].harga := StrToInt (copy(temp,1,guardMark-2));
			{Membaca durasi kadaluarsa bahan mentah yang bertipe integer}
			temp := copy (temp,guardMark+2, length(temp));
			bahanMentah.tab[j].kadaluarsa := StrToInt (copy(temp,1,guardMark-2));
		end;
		bahanMentah.neff:=j;
		close (inf);
	end;
	...
	begin {ALGORITMA UTAMA LoadResep}
		...
		begin
			...
			begin
				...
				end else {Terdapat bahan mentah dan bahan olahan yang memiliki nama sama, sehingga tidak sesuai spesifikasi}
				begin
					writeln ('File resep baris ke ', j, ' memiliki bahan penyusun yang tidak terdefinisi (terdapat bahan mentah dan bahan olahan yanng bernama sama).');
				end;
				temp := copy (temp,guardMark+2, lTemp); {Jika guardMark+2 lebih dari ltemp, maka copy () akan mengembalikan string kosong}
				lTemp := length(temp);
			end;
